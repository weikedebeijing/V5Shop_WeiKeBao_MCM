            ...
            if (res.length != 2) {//等于2时是空数组“[]”
                ...
                for (var i = 0; i < jsonRes.length; i++) {
                    ...
                    if (parseInt($currentObj.PropertyID) != 20000) {//20000是淘宝的“品牌”这一属性的ID，品牌做特殊处理，不是品牌的做下面的循环
                        ...
                        } else if (parseInt($currentObj.PropertyID) == 6939376) {//食品类
                            
                            $propertyElement = $("<li class='list-li'><div class='float-l w130'>      <span class='star'>*</span><label>" + $currentObj.PropertyName + "</label></div><div class='float-l'><select  class='w230' id='property" + $currentObj.PropertyID + "' onchange='OperateStarMark(event),GetSubProperty(event," + categoryID + ")' onpropertychange='OperateStarMark(event),GetSubProperty(event," + categoryID + ")'><option></option></select></div><div class='float-l'><span class='prompt'></span></div></li>");
                            $("#foodSecurityContainer").css("display", "block");
                        } else if (parseInt($currentObj.PropertyID) == 6932208) {//(婴幼儿)奶粉等
                            
                            $propertyElement = $("<li class='list-li'><div class='float-l w130'>      <span class='star'>*</span><label>" + $currentObj.PropertyName + "</label></div><div class='float-l'><select  class='w230' id='property" + $currentObj.PropertyID + "'><option></option></select></div><div class='float-l'><span class='prompt'></span></div></li>");
                            $("#foodSecurityContainer").css("display", "block");
                        ...
                        $.ajax({
                            ...
                            success: function (res2) {
                                ...
                                for (var j = 0; j < jsonRes2.length; j++) {
                                    var $brandValue = $("<div id='" + jsonRes2[j].PropertyValueID + "' style='width:100%;height:20px;line-height:20px;' onclick='SelectThisBrand(event)' isParent='" + jsonRes2[j].IsParent + "'>" + jsonRes2[j].ValueName + "</div>")
                                    //alert(jsonRes2[j].ValueName);
                                    $("#dvBrandPropertyValue").append($brandValue);
                                }
                            },
                            error: function () {
                        ...
                        $.ajax({
                            ...
                            success: function (res2) {
                                ...
                                for (var j = 0; j < jsonRes2.length; j++) {
                                    ...
                                    $("#property" + jsonRes2[j].PropertyID + "").append($option2);
                                }
                            },
                            error: function () {
                                $(".Loading").removeClass("style0green style0yellow").addClass("style0red");
                        ...
                        $.ajax({
                            ...
                            success: function (res2) {
                                ...
                                for (var j = 0; j < jsonRes2.length; j++) {
                                    ...
                                    $("#property" + jsonRes2[j].PropertyID + "").append($checkbox);
                                }
                            },
                            error: function () {
                                $(".Loading").removeClass("style0green style0yellow").addClass("style0red");
                        ...
                        $.ajax({
                            ...
                            success: function (res2) {
                                ...
                                for (var j = 0; j < jsonRes2.length; j++) {
                                    ...
                                    $("#property" + jsonRes2[j].PropertyID + "").append($radio);
                                }
                            },
                            error: function () {
                                $(".Loading").removeClass("style0green style0yellow").addClass("style0red");
...
function InitSelectThisSpec(containerID) {
    ...
    $("." + containerID + " .specCombineTable tr:not(:first)").remove();
    $("." + containerID + " .specCombineTable tr:first td").remove();
}
...
function SelectThisSpec(event, categoryID, containerID) {
    if (containerID != "specContainerForDestribution") { //-----------------------------------------------不是分销的规格组合表
        ...
        if (eventSrc != undefined) {
            var $specID = $(eventSrc).parent().parent().attr("id").substring(4);
            if ($specID == "1627207") {
                var flag = 0;
        ...
        $("." + containerID + " #" + containerID + " .specName").each(function () {
            ...
            if (clickTimes == 1) {
                ...
                $("." + containerID + " .specCombineTable tr:first").append(td);
            }
        });

        if (flag == flag3 && createFlag == 0) {//flag == flag3 表示所有的规格都选全了 createFlag == 0表示还没有创建过表头行
            var td = $("<td style='display:none;'>SKU标签</td><td>价格<input type='checkbox' onclick='SetSamePrice(event,\"" + containerID + "\")'/></td><td>数量</td><td>商家编码</td>");
            $("." + containerID + " .specCombineTable tr:first").append(td);
            createFlag = 1;
        }

        if (flag == flag3) { //所有的规格信息都至少选择了一项，让规格编辑表格显示，否则隐藏
            $("." + containerID + " .specCombineTable").css("display", "block");
        } else {
            $("." + containerID + " .specCombineTable").css("display", "none");
            $(".prompt", $("." + containerID + " .specCombineTable").parents("li")).text(""); //表格隐藏时将提示信息清空
        }
            ...
            if ($(eventSrc).attr("checked") == "checked") {
                ...
                if (v1.length > 0 && v2.length == 0) {//只有一个规格
                    for (var i = 0; i < v1.length; i++) {
                        ...
                        existed = false;
                        $("." + containerID + " .specCombineTable tr td[id]").each(function () {
                            if ($(this).attr("id") == v1[i].key) existed = true;
                        })
                        if (existed == false) {
                            var tr = $("<tr><td id='" + v1[i].key + "' specID='" + v1[i].specID + "' class='specMark'>" + v1[i].value + "</td><td style='display:none;' id='" + v1[i].key + "' class='specConbineMark'>" + v1[i].value + "</td><td><input id='txtPrice' type='text' /></td><td><input id='txtCount' type='text' onblur='SumProductCount(\"" + containerID + "\")' /></td><td><input id='txtProductCode' type='text' /><input class='thisSKUCode' type='hidden' /></td></tr>");
                            $("." + containerID + " .specCombineTable").append(tr);
                        }
                    }
                    ...
                    for (var i = 0; i < v1.length; i++) {
                        for (var j = 0; j < v2.length; j++) {
                            ...
                            if (existed == false) {
                                var tr = $("<tr><td id='" + v1[i].key + "' specID='" + v1[i].specID + "' class='specMark'>" + v1[i].value + "</td><td id='" + v2[j].key + "' specID='" + v2[j].specID + "' class='specMark'>" + v2[j].value + "</td><td style='display:none;' id='" + v1[i].key + "-" + v2[j].key + "' class='specConbineMark'>" + v1[i].value + "-" + v2[j].value + "</td><td><input id='txtPrice' type='text' onblur='PriceFormat(event)' /></td><td><input id='txtCount' type='text' onblur='SumProductCount(\"" + containerID + "\")' /></td><td><input id='txtProductCode' type='text' /><input class='thisSKUCode' type='hidden' /></td></tr>");
                                $("." + containerID + " .specCombineTable").append(tr);
                            }
                        }
                    }
                    ...
                    for (var i = 0; i < v1.length; i++) {
                        for (var j = 0; j < v2.length; j++) {
                            for (var k = 0; k < v3.length; k++) {
                                ...
                                if (existed == false) {
                                    var tr = $("<tr><td id='" + v1[i].key + "' specID='" + v1[i].specID + "' class='specMark'>" + v1[i].value + "</td><td id='" + v2[j].key + "' specID='" + v2[j].specID + "' class='specMark'>" + v2[j].value + "</td><td id='" + v3[k].key + "' specID='" + v3[k].specID + "' class='specMark'>" + v3[k].value + "</td><td style='display:none;' id='" + v1[i].key + "-" + v2[j].key + "-" + v3[k].key + "' class='specConbineMark'>" + v1[i].value + "-" + v2[j].value + "-" + v3[k].value + "</td><td><input id='txtPrice' type='text' onblur='PriceFormat(event)' /></td><td><input id='txtCount' type='text' onblur='SumProductCount(\"" + containerID + "\")'/></td><td><input id='txtProductCode' type='text' /><input class='thisSKUCode' type='hidden' /></td></tr>");
                                    $("." + containerID + " .specCombineTable").append(tr);
                                }
                            }
                        }
                ...
                if ($("." + containerID + " .specCombineTable").css("display") == "block") {
    ...
    if ($(eventSrc).attr("isallowalias") == "true") {//是否支持自定义规格别名
        if ($(eventSrc).attr("checked") == "checked") {
            //点选一个规格值，将其值可编辑（span隐藏，input文本框显示）
            $("span", $(eventSrc).parent()).css("display", "none");
            $("input[type=text]", $(eventSrc).parent()).css("display", "inline");
        } else {
            //取消一个规格值，将其值可编辑（span显示，input文本框隐藏）
            $("span", $(eventSrc).parent()).css("display", "inline");
    ...
    //规格组合表所在ul的显示和隐藏控制
    if ($("." + containerID + " .specCombineTable").css("display") == "block") {
        $("." + containerID + " .specCombineTable").parents("ul").show();
    } else {
        $("." + containerID + " .specCombineTable").parents("ul").hide();
    }
}
...
function SetSamePrice(event, containerID) {
    ...
    if ($(eventSrc).attr("checked") == "checked") {//是选中的时候才去设置同样的值
        if ($(eventSrc).attr("id") == "cbxDaixiaoPrice") {
            var basePrice = $("." + containerID + " .specCombineTable #txtDaixiaoPrice:first").val();
            var reg = /\./g;
            if (!reg.test(basePrice)) {
                basePrice = basePrice + ".00";
            }
            $("." + containerID + " .specCombineTable #txtDaixiaoPrice").each(function () {
                $(this).val(basePrice);
            })
        } else if ($(eventSrc).attr("id") == "cbxJingxiaoPrice") {
            var basePrice = $("." + containerID + " .specCombineTable #txtJingxiaoPrice:first").val();
            var reg = /\./g;
            if (!reg.test(basePrice)) {
                basePrice = basePrice + ".00";
            }
            $("." + containerID + " .specCombineTable #txtJingxiaoPrice").each(function () {
                $(this).val(basePrice);
            })
        } else if ($(eventSrc).attr("id") == "cbxLingshouPrice") {
            var basePrice = $("." + containerID + " .specCombineTable #txtLingshouPrice:first").val();
            var reg = /\./g;
            if (!reg.test(basePrice)) {
                basePrice = basePrice + ".00";
            }
            $("." + containerID + " .specCombineTable #txtLingshouPrice").each(function () {
                $(this).val(basePrice);
            })
        } else {
            var basePrice = $("." + containerID + " .specCombineTable #txtPrice:first").val();
            var reg = /\./g;
            if (!reg.test(basePrice)) {
                basePrice = basePrice + ".00";
            }
            $("." + containerID + " .specCombineTable #txtPrice").each(function () {
                $(this).val(basePrice);
            })
        }
    }
...
function SumProductCount(containerID) {
    if (containerID == "specContainer") {
        var sumCount = 0;
        $("." + containerID + " .specCombineTable tr input[id=txtCount]").each(function () {
...
function SelectBrandValue(event) {
    stopBubble(event);
    if ($("#dvBrandPropertyValue div").length > 0) {//有可选择的品牌才显示
        $("#dvBrandPropertyValue").css("display", "block");
    }
...
function UserDefineBrand(event) {
    ...
    var flag = 0;
    var tempID = 0;
    $("#dvBrandPropertyValue div").each(function () {
...
function SelectThisBrand(event) {
    ...
    $("#brandProperty20000").val(brandName);
    $("#brandProperty20000").attr("brandID", brandID);
    $("#dvBrandPropertyValue").css("display", "none");
    $("#brandProperty20000").change();
}

document.onclick = function myfunction() {
    $("#dvBrandPropertyValue").css("display", "none");
    if ($("#brandProperty20000").val() == "") {
        $("#brandProperty20000").val("可直接输入或选择");
        $("#brandProperty20000").css("color", "Gray");
    } else if ($("#brandProperty20000").val() == "可直接输入或选择") {
        $("#brandProperty20000").css("color", "Gray");
    }
    else {
        $("#brandProperty20000").css("color", "#222222");
    }
}
    ...
    if (eventSrc != undefined) {
        ...
        if (currentSpecValue != "" && currentSpecValue != oldSpecValue) {
            ...
            if ($("input[type=checkbox]", $(eventSrc).parent()).attr("checked") == "checked") {
                ...
                $("." + containerID + " .specCombineTable tr:not(:first)").each(function () {
                    var temp = "";
...
function GetSubProperty(event, categoryID) {
    ...
    var eventSrc = event.srcElement ? event.srcElement : event.target;
    if (eventSrc != undefined) {
        var propertyID = $(eventSrc).attr("id").replace("property", "");
        ...
        for (var i = 0; i < jsonPropertyByCategoty.length; i++) {
            if (jsonPropertyByCategoty[i].ParentID == propertyID && jsonPropertyByCategoty[i].ParentVID == propertyValueID) {
                var $subProperty = $("<span class='subPropertyMark' style='margin-left:15px;'>" + jsonPropertyByCategoty[i].PropertyName + "</span><select class='subPropertyMark' id='property" + jsonPropertyByCategoty[i].PropertyID + "' onchange='GetSubSubProperty(event," + categoryID + ")' onpropertychange='GetSubSubProperty(event," + categoryID + ")'></select>");
                        ...
                        $.ajax({
                            ...
                            success: function (res3) {
                                ...
                                for (var k = 0; k < jsonRes3.length; k++) {
                                    ...
                                    $("#property" + jsonRes3[k].PropertyID + "").append($option3);
                                }
                            },
                            error: function () {
...
function GetSubSubProperty(event, categoryID) {
    ...
    if (eventSrc != undefined) {
        ...
        if (subPropertyIDMark != propertyID) {//兼容IE8使用, 当二级属性和三级属性的eventSrc的propertyID相等时,说明事件源识别错误,不加载三级属性
            ...
            for (var i = 0; i < jsonPropertyByCategoty.length; i++) {
                if (jsonPropertyByCategoty[i].ParentID == propertyID && jsonPropertyByCategoty[i].ParentVID == propertyValueID) {
                    var $subProperty = $("<span class='subSubPropertyMark'>&nbsp;" + jsonPropertyByCategoty[i].PropertyName + "</span><select class='subSubPropertyMark' id='property" + jsonPropertyByCategoty[i].PropertyID + "'></select>");
    ...
    if (eventSrc != undefined) {
        ...
        for (var i = 0; i < jsonPropertyByCategoty.length; i++) {
            if (jsonPropertyByCategoty[i].ParentID == propertyID && jsonPropertyByCategoty[i].ParentVID == parentVID) {
                var $subProperty = $("<span class='subPropertyMarkForBrand' style='margin-left:15px;'>" + jsonPropertyByCategoty[i].PropertyName + "</span><input type='text'  class='subPropertyMarkForBrand' id='property" + jsonPropertyByCategoty[i].PropertyID + "' ></input>");
                $(".subPropertyMarkForBrand", $(eventSrc).parent()).remove();
                $(eventSrc).parent().append($subProperty);
                break;
            }
...
function GetChildTemplate(event, jsonRes) {
    ...
    if ($(eventSrc).val() == "" || $(eventSrc).val() == "可直接输入或选择") {
        ...
        if ($("input[class=childProperty]", $(eventSrc).parent()).val() == undefined) {
            ...
            for (var i = 0; i < jsonRes.length; i++) {
                if (jsonRes[i].PropertyID == 20000 && jsonRes[i].ChildTemplate != "") {
